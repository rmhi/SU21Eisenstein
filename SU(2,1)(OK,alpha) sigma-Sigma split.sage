...
class ArithGp:
    def __init__(self,gp_name,coset_key,latex_name=None):
        ...
        try:
            ...
            self._gap_gens = data[9]
            self._gap_relns = data[10]
            self._no_of_gap_gens = data[11]
            self._no_of_gap_relns = data[12]
            try:
                self._reln_matrix_smith = data[13]
            except IndexError:
    ...
    def extended_reln_matrix(self, verbose = False, cocycle = sigma, force=False):
        if self._extended_reln_matrix !=None and not force:
            if verbose:
                print('Already have this.')
    ...
    def H_1(self,verbose=False):    # first homology group
        if self._H_1 == None:
            if self._reln_matrix_smith == None:
    ...
    def H_1_met(self,verbose=False):    # first homology of pre-image in covering group
        if self._H_1_met == None:
            if self._extended_reln_matrix_smith == None:
    ...
    def H_1_diamond_operator(self,g,h1elt):
        h1elt_lift = self.H_1_lift(h1elt)
        h1elt_lift_conj = SUalpha2PSUalpha(g^-1*h1elt_lift*g) # project into self as a quotient.
        h1elt_conj = self.elt2H_1(h1elt_lift_conj)
        return h1elt_conj
...
# beta must be a prime power, otherwise the coset key will crash.
# for other values of beta, define the group to be the intersection of groups for prime power factors of beta.
def Gamma0(beta):
    if len(factor(K(beta))) > 1:
        print('Error in Gamma0(beta). You have used beta=',beta,' but this is not a prime power.')
        print('Instead use the intersection of various groups Gamma0(beta) to construct this group.')
        raise ValueError
        return None
...
print('\n\n\nDefining G to be the group PSU(2,1)(OK,alpha)')
G = Gamma0(1)

print('Defining the cocycle Sigma on G to be the cup product of the homomorphism phi and its complex conjugate')
def Sigma(g,h):
    return (K((phi(g) * K(phi(h)).conjugate())/alpha)).trace()

print('Defining test_cocycle = 4*sigma-Sigma')
def test_cocycle(g,h):
    return 4*sigma(g,h) - Sigma(g,h)

print('Calculating the abelianization of the central extension of G corresponding to test-cocycle.')
print('Each row of the following matrix gives a relation.')
print('The first 5 columns correspond to lifts of the generators of G.')
print('The last column corresponds to a generator of the centre.')
G.extended_reln_matrix(cocycle = test_cocycle, force=True)
print(G.reduced_extended_reln_matrix())
